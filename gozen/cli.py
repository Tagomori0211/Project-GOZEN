...
例:
  # 順次実行（Pro推奨）
  python cli.py --mode sequential task.yaml
  
  # 並列実行（Max 5x推奨）
  python cli.py --mode parallel --plan max5x task.yaml
...
def run_interactive(args):
    ...
    while True:
        try:
            ...
            task = {
                'task_id': f"INTERACTIVE-{__import__('datetime').datetime.now().strftime('%Y%m%d%H%M%S')}",
                'mission': mission,
                'requirements': []
            }
