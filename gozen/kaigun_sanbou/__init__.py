...
class KaigunSanbou:
    ...
    async def create_proposal(self, task: dict) -> dict:
        """
        タスクに対する提案を作成
    ...
    def _generate_summary(self, mission: str, requirements: list) -> str:
        """提案の要約を生成"""
        return f"""
【海軍参謀の提案】
...
要件: {', '.join(requirements) if requirements else '未指定'}
"""
    
    def _design_architecture(self, mission: str, requirements: list) -> dict:
        """理想的なアーキテクチャを設計"""
        return {
            'type': 'ideal',
            'components': [
    ...
    def _extract_key_points(self, requirements: list) -> list:
        """重要ポイントを抽出"""
        return [
            '完全自動化による運用負荷ゼロ',
            '将来の200ユーザー対応を想定',
    ...
    def _estimate_timeline(self, requirements: list) -> dict:
        """タイムライン見積もり"""
        return {
...
async def create_proposal(task: dict) -> dict:
    """提案を作成（モジュールレベル関数）"""
    sanbou = get_instance()
