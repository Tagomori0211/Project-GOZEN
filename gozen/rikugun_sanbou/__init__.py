...
class RikugunSanbou:
    ...
    async def create_objection(self, task: dict, proposal: dict) -> dict:
        """
        海軍提案に対する異議を作成
    ...
    def _generate_objection_summary(self, task: dict, proposal: dict) -> str:
        """異議の要約を生成"""
        return f"""
【陸軍参謀の異議】
...
"""
    ...
    def _identify_concerns(self, proposal: dict) -> list:
        """懸念点を特定"""
        return [
            {
                'category': '過剰設計',
                'detail': '現在50ユーザーに対してk3sクラスタは過剰',
                'severity': 'high'
            },
            {
                'category': 'コスト',
                'detail': '初期構築コスト¥20,000超過の見込み',
                'severity': 'medium'
            },
            {
                'category': '運用負荷',
                'detail': '1人での管理は学習曲線的に困難',
                'severity': 'high'
            },
            {
                'category': '複雑性',
                'detail': 'トラブルシューティングが困難',
                'severity': 'medium'
            }
        ]
    
    def _propose_alternative(self, task: dict, proposal: dict) -> dict:
        """代替案を提案"""
        return {
            'title': '段階的アプローチ',
            'phase1': {
    ...
    def _extract_key_points(self, proposal: dict) -> list:
        """重要ポイントを抽出"""
        return [
            '現状の要件をまず満たす',
            '段階的な投資でリスク分散',
    ...
    def _suggest_compromise(self, proposal: dict) -> dict:
        """折衷案を提案"""
        return {
            'accept_from_kaigun': [
                'Ansibleによる自動化',
    ...
    async def verify_with_gemini(self, proposal: dict) -> dict:
        """
        Gemini APIを使用して提案を検証（オプション）
        """
        if not self.gemini_enabled:
            return {'status': 'skipped', 'reason': 'Gemini API not configured'}
...
async def create_objection(task: dict, proposal: dict) -> dict:
    """異議を作成（モジュールレベル関数）"""
    sanbou = get_instance()
